<template>
	<view>
		...
		<u-navbar title="Q&A" :border-bottom="true" :is-back="param.form != 'h5'">
		...
		<view>
			...
			<view class="p30">
				...
				<view v-if="param && param.fromType == 'qa'">
					<!-- 进行中 -->
					<view class="flex itc mb20" v-if="info.resolved_status == 1">
						<view class="mr10">
							<image src="../../static/images/qa1.jpg" mode="" class="jifen"></image>
						</view>
						<view class="fz26" style="color: #ffd03a;">{{info.integer_sum}}</view>
					</view>
					<!-- 已完成 -->
				...
				<!-- 图片 -->
				<view>
					<image :src="url + item.url" mode="widthFix" style="width: 100%;"
						v-for="(item,index) in info.pics " @click="previewImage(index, info.pics)"></image>
				</view>
				<!-- 底部输入框 -->
				<view class="footer">
					...
					<view class="footer_box ">
						<view class="flex p0_30 itc h100">
							<view class="inp fz24 c9 flex_1 mr20" @click="showComment">Say something</view>
		...
		<view>
			...
			<commentList :commentDataList="commentDataList" @childComment="childComment" @refreshInfo="refreshInfo" :info="info"></commentList>
...
<script>
	// resolved_status == 2 表示问题已完成
	import commentList from '@/components/comment/qa.vue';
	export default {
		data() {
			return {
				show: false, // 评论输入框标志
		...
		methods: {
			...
			submit() {
				...
				if (this.childCommentData.question_answer_id) {
					datas.pid = this.childCommentData.question_answer_id;
				}
				uni.showLoading({
					mask: true,
					title: "Loading"
				...
				this.requestSet.getData(datas, '/V1.QuestionAnswer/save', 'POST').then((res) => {
					if (res.status == 200) {
			...
			getCommentList() {
				...
				this.requestSet.getData(datas, '/V1.QuestionAnswer/list', 'GET').then((res) => {
					if (res.status == 200) {
			...
			delInfo() {
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) {
							var datas = {
								question_ids: this.param.id,
							};
							this.requestSet.getData(datas, '/V1.Question/delete', 'POST').then((
							res) => {
								if (res.status == 200) {
									// this.commentDataList = this.commentDataList.concat(res.data.list);
									this.$r.showToast('Success');
