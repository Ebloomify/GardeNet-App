	...
	export default {
		...
		methods: {
			...
			submit() {
				...
				var datas = {
					community_article_id: this.param.community_article_id,
					content: this.desc,
					pid: ""
				};
				if (this.childCommentData.community_comment_id) {
					...
					datas.pid = this.param.community_comment_id;
				};
				uni.showLoading({
					mask: true,
					title: "Loading"
			...
			topLike() {
				...
				var datas = {
					type: 5,
					object_id: this.param.community_comment_id
				};
			...
			getLike(index, item) {
				...
				var datas = {
					type: 5,
					object_id: item.community_comment_id
				};
				this.requestSet.getData(datas, '/V1.MemberLike/addAndRemove', 'POST').then((res) => {
					if (res.status == 200) {
			...
			getList() {
				var datas = {
					community_article_id: this.param.community_article_id,
					pid: this.param.community_comment_id,
					page: 1,
					limit: 10000
				};
