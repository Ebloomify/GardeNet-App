...
class MemberService extends CommonService {
 ...
 /*start*/
    ...
    {
        try {
            validate(\app\api\validate\V1\Member::class)->scene('register')->check($data);
 ...
 /*start*/
    public static function emailLogin($data, $returnField)
    {
        $where['email'] = $data['email'];
        $pwds = md5($data['password'] . config('my.password_secrect'));
        try {
            $res = Member::field($returnField)->where($where)->find();
            if(empty($res)){
                throw new ValidateException('该账号尚未注册');
            }
            if($res['password']!=$pwds){
                throw new ValidateException('您输入的密码错误');
            }
            if($res['status']!=1){
                throw new ValidateException('您的账户已禁用');
            }
            if($data['client_id']){
                db('member')->where('member_id',$res['member_id'])->update(['client_id'=>$data['client_id']]);
            }
        } catch (\Exception $e) {
            abort(config('my.error_log_code'), $e->getMessage());
        }
 ...
 /*start*/
    ...
    {
        try {
            validate(\app\api\validate\V1\Member::class)->scene('update')->check($data);
 ...
 /*start*/
    public static function permissionStatus($where, $data)
    {
        try {
            $res = Member::where($where)->update($data);
            if($data['member_permission_status']==1){
                IntegerService::plus(8, $where['member_id']);
            }
        } catch (ValidateException $e) {
            throw new ValidateException ($e->getError());
        } catch (\Exception $e) {
 ...
 /*start*/
    public static function resetPassword($where, $password)
    {
        try {
 ...
 /*start*/
    public static function createDefaultData($email, $password)
    {
        $bool = false;
        do{
            $invite_code = self::getRandomString(8);
            $member_invitation_code = \app\api\model\V1\Member::where('invitation_code',$invite_code)->count();
            if($member_invitation_code)$bool=true;
        }while($bool);

        $data = [
            'avatar' => db('config')->where(['name' => 'member_default_avatar'])->value('data'),
            'nickname' => 'Gardener_'.random(10),
            'email' => $email,
            'sex' => 1,
            'mobile' => '',
            'area' => '',
            ...
            'status' => 1,
            'create_time' => time(),
            'password' => md5($password . config('my.password_secrect')),
            'integral' => 0,
            'all_integral' => 0,
            'member_level' => 1,
            'member_level_exp' => 0,
