...
// 		if(!$_FILES) throw new ValidateException('上传验证失败');
	...
	*/
	...
	/*start*/
    /**
     * @api {post} /Base/emailCode 03、发送邮箱验证码
     * @apiGroup Base
     * @apiVersion 1.0.0
     * @apiDescription  邮箱验证码
	...
	public function emailCode()
    {
        ...
        if (! filter_var($email,FILTER_VALIDATE_EMAIL)){
            return json(['status'=>201,'msg'=>'邮箱非法']);

        }

        $handler = new \tools\Email($email);

        $code = mt_rand(100000,999999);

        cache($email,$code,600);
        $handler->send("GardeNet App",$code);
