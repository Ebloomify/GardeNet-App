<template>
	<view class="wrap">
		...
		<view class="footer  ">
			...
			<view class="footer_box bt1 bgf">
				<view class="flex p0_30 itc h100">
					<view class="inp fz24 c9 flex_1 " @click="showComment">Say something</view>
				</view>
			</view>
		</view>
		<!-- 评论层 -->
		...
		<view class="commentBox bgf trans" :class="{'showCommentBox' :show }">
			...
			<!--  -->
			<view class="p30">
				<textarea value="" maxlength="-1" placeholder="Say something" class="fz26 c3" v-model="desc"></textarea>
			</view>
			<!--  -->
		...
		methods: {
			...
			submit() {
				...
				if (this.childCommentData.question_answer_id) {
					...
					datas.pid = this.param.question_answer_id;
				};
				uni.showLoading({
					mask: true,
					title: "Loading"
			...
			topLike() {
				...
				var datas = {
					type: 6,
					object_id: this.param.question_answer_id
				};
			...
			getList() {
				var datas = {
					question_id: this.param.question_id,
					pid: this.param.question_answer_id,
					page: 1,
					limit: 10000
				};
